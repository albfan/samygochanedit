...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (; args.length > firstArg; firstArg++) { 
			if      (args[firstArg].equals("-d")) { tempDir  = "" + args[++firstArg]; }
			else if (args[firstArg].equals("-c")) { chanFile = "" + args[++firstArg]; }
		...
		if (Main.tempDir == null) {
			// tmpDir = System.getProperty("java.io.tmpdir") + "SamyGoTemp" + File.separator;
			tmpDir = System.getProperty("user.home") + File.separator + "SamyGoTemp" + File.separator; // in Win7 temp dir is hidden!!!
		} else {
...
//		display.post(SWT.CTRL + 'O');
		...
		if (Main.chanFile != null) {
			File d;
			if (Main.scmFile != null) d = new File(tempDir+Main.chanFile);
			else                      d = new File(Main.chanFile);
			if (d.isFile()) { 
